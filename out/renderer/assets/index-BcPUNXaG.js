  ...
  if (type === 'clear') {
    ...
    if (targetIsArray && key2 === 'length') {
      ...
      depsMap.forEach((dep, key22) => {
        if (
          key22 === 'length' ||
          key22 === ARRAY_ITERATE_KEY ||
          (!isSymbol(key22) && key22 >= newLength)
        ) {
          run(dep)
        }
      })
...
class MutableReactiveHandler extends BaseReactiveHandler {
  ...
  set(target, key2, value, receiver) {
    ...
    const hadKey =
      isArray$1(target) && isIntegerKey(key2)
        ? Number(key2) < target.length
        : hasOwn(target, key2)
...
function createInstrumentations(readonly2, shallow) {
  const instrumentations = {
    ...
    has(key2) {
      ...
      return key2 === rawKey
        ? target.has(key2)
        : target.has(key2) || target.has(rawKey)
    },
...
function createInstrumentationGetter(isReadonly2, shallow) {
  ...
  return (target, key2, receiver) => {
    ...
    return Reflect.get(
      hasOwn(instrumentations, key2) && key2 in target
        ? instrumentations
        : target,
      key2,
      receiver
...
const shallowUnwrapHandlers = {
  get: (target, key2, receiver) =>
    key2 === '__v_raw' ? target : unref(Reflect.get(target, key2, receiver)),
  set: (target, key2, value, receiver) => {
    ...
    callWithErrorHandling(appWarnHandler, instance, 11, [
      ...
      msg +
        args
          .map(a2 => {
            var _a, _b
            return (_b = (_a = a2.toString) == null ? void 0 : _a.call(a2)) !=
              null
              ? _b
              : JSON.stringify(a2)
          })
...
function resolveTransitionHooks(vnode, props, state, instance, postClone) {
  ...
  const hooks = {
    ...
    clone(vnode2) {
      const hooks2 = resolveTransitionHooks(vnode2, props, state, instance)
      return hooks2
    },
  }
  return hooks
  ...
  if (sourceIsArray || isString$1(source)) {
    ...
    for (let i2 = 0, l2 = source.length; i2 < l2; i2++) {
      ret[i2] = renderItem(
        needsWrap
          ? isReadonlySource
            ? toReadonly(toReactive(source[i2]))
            : toReactive(source[i2])
          : source[i2],
        i2,
        void 0,
        cached
    ...
    if (source[Symbol.iterator]) {
      ret = Array.from(source, (item, i2) =>
        renderItem(item, i2, void 0, cached)
      )
    } else {
...
function createWatcher(raw, ctx, publicThis, key2) {
  let getter = key2.includes('.')
    ? createPathGetter(publicThis, key2)
    : () => publicThis[key2]
  if (isString$1(raw)) {
...
const isInternalKey = key2 =>
  key2 === '_' || key2 === '__' || key2 === '_ctx' || key2 === '$stable'
...
function invalidateMount(hooks) {
  if (hooks) {
    for (let i2 = 0; i2 < hooks.length; i2++) hooks[i2].flags |= 8
  }
}
...
function isEmitListener(options, key2) {
  ...
  return (
    hasOwn(options, key2[0].toLowerCase() + key2.slice(1)) ||
    hasOwn(options, hyphenate(key2)) ||
    hasOwn(options, key2)
  )
}
  ...
  if (optimized && patchFlag >= 0) {
    ...
    if (patchFlag & 16) {
      ...
      for (let i2 = 0; i2 < dynamicProps.length; i2++) {
        ...
        if (
          nextProps[key2] !== prevProps[key2] &&
          !isEmitListener(emits, key2)
        ) {
          return true
        }
      }
...
function hasPropsChanged(prevProps, nextProps, emitsOptions) {
  ...
  for (let i2 = 0; i2 < nextKeys.length; i2++) {
    ...
    if (
      nextProps[key2] !== prevProps[key2] &&
      !isEmitListener(emitsOptions, key2)
    ) {
      return true
    }
  }
...
const normalizeRef = ({ ref: ref3, ref_key, ref_for }) => {
  ...
  return ref3 != null
    ? isString$1(ref3) || isRef(ref3) || isFunction(ref3)
...
function setupStatefulComponent(instance, isSSR) {
  ...
  if (setup2) {
    ...
    const setupContext = (instance.setupContext =
      setup2.length > 1 ? createSetupContext(instance) : null)
    const reset = setCurrentInstance(instance)
    const setupResult = callWithErrorHandling(setup2, instance, 0, [
      instance.props,
      setupContext,
    ])
...
function patchAttr(
  ...
  isSVG,
  instance,
  isBoolean = isSpecialBooleanAttr(key2)
) {
...
const isNativeOn = key2 =>
  key2.charCodeAt(0) === 111 &&
  key2.charCodeAt(1) === 110 && // lowercase letter
  key2.charCodeAt(2) > 96 &&
  key2.charCodeAt(2) < 123
const patchProp = (
  el,
  key2,
  prevValue,
  ...
  if (key2 === 'class') {
    ...
    key2[0] === '.'
      ? ((key2 = key2.slice(1)), true)
      : key2[0] === '^'
        ? ((key2 = key2.slice(1)), false)
        : shouldSetAsProp(el, key2, nextValue, isSVG)
  ) {
    patchDOMProp(el, key2, nextValue)
    if (
      !el.tagName.includes('-') &&
      (key2 === 'value' || key2 === 'checked' || key2 === 'selected')
    ) {
    ...
    el._isVueCE &&
    (/[A-Z]/.test(key2) || !isString$1(nextValue))
  ) {
...
const useProviderDisabled = disabled => {
  ...
  provide(
    ...
    computed(() => {
      var _a
      return (_a = disabled.value) !== null && _a !== void 0
        ? _a
...
function murmur2(str) {
  ...
  for (; len >= 4; ++i2, len -= 4) {
    k2 =
      (str.charCodeAt(i2) & 255) |
      ((str.charCodeAt(++i2) & 255) << 8) |
      ((str.charCodeAt(++i2) & 255) << 16) |
      ((str.charCodeAt(++i2) & 255) << 24)
    k2 =
      /* Math.imul(k, m): */
  ...
  if (typeof document !== 'undefined' && document.head && document.body) {
    ...
    Array.from(document.querySelectorAll(`style[${ATTR_MARK}]`)).forEach(
      style => {
        ...
        if (styleHash[hash]) {
          if (style[CSS_IN_JS_INSTANCE] === cssinjsInstanceId) {
            ;(_a = style.parentNode) === null || _a === void 0
              ? void 0
              : _a.removeChild(style)
          }
        } else {
          styleHash[hash] = true
        }
...
const getCache = () => {
  ...
  if (instance && instance.appContext) {
    const globalCache =
      (_c =
        (_b =
          (_a = instance.appContext) === null || _a === void 0
            ? void 0
            : _a.config) === null || _b === void 0
          ? void 0
          : _b.globalProperties) === null || _c === void 0
        ? void 0
        : _c.__ANTDV_CSSINJS_CACHE__
    if (globalCache) {
      cache = globalCache
    } else {
      cache = createCache()
...
const useStyleInject = () => {
  ...
  return inject(
    StyleContextKey,
    shallowRef(
      _extends(_extends({}, defaultStyleContext), {
        cache,
      })
    )
  )
...
function useClientCache(prefix, keyPath, cacheFn, onCacheRemove) {
  ...
  const clearCache = pathStr => {
    styleContext.value.cache.update(pathStr, prevCache => {
      ...
      if (nextCount === 0) {
        onCacheRemove === null || onCacheRemove === void 0
          ? void 0
...
function findStyles$1(container) {
  return Array.from(
    (containerCache$1.get(container) || container).children
...
function injectCSS$1(css) {
  ...
  if (prepend) {
    if (prepend === 'queue') {
      const existStyle = findStyles$1(container).filter(node2 =>
        ['prepend', 'prependQueue'].includes(node2.getAttribute(APPEND_ORDER$1))
      )
...
function findExistNode$1(key2) {
  let option =
    arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}
  const container = getContainer$2(option)
  return findStyles$1(container).find(
    node2 => node2.getAttribute(getMark$1(option)) === key2
  )
}
...
function updateCSS$1(css, key2) {
  ...
  if (existNode) {
    if (
      ((_a = option.csp) === null || _a === void 0 ? void 0 : _a.nonce) &&
      existNode.nonce !==
        ((_b = option.csp) === null || _b === void 0 ? void 0 : _b.nonce)
    ) {
      existNode.nonce =
        (_c = option.csp) === null || _c === void 0 ? void 0 : _c.nonce
    }
...
class ThemeCache {
  ...
  internalGet(derivativeOption) {
    ...
    derivativeOption.forEach(derivative2 => {
      ...
      if (!cache) {
        ...
        cache =
          (_a = cache === null || cache === void 0 ? void 0 : cache.map) ===
            null || _a === void 0
            ? void 0
            : _a.get(derivative2)
      }
    ...
    if (
      (cache === null || cache === void 0 ? void 0 : cache.value) &&
      updateCallTimes
    ) {
  ...
  get(derivativeOption) {
    var _a
    return (_a = this.internalGet(derivativeOption, true)) === null ||
      _a === void 0
      ? void 0
      : _a[0]
  }
...
class Theme {
  ...
  getDerivativeToken(token2) {
    return this.derivatives.reduce(
      (result, derivative2) => derivative2(token2, result),
      void 0
    )
  }
}
...
function supportSelector(styleStr, handleElement, supportCheck) {
  ...
  if (canUseDom$1()) {
    ...
    ele.style.left = '0'
    ele.style.top = '0'
    handleElement === null || handleElement === void 0
      ? void 0
      : handleElement(ele)
    document.body.appendChild(ele)
    const support = supportCheck
      ? supportCheck(ele)
      : (_a = getComputedStyle(ele).content) === null || _a === void 0
        ? void 0
        : _a.includes(checkContent)
    ;(_b = ele.parentNode) === null || _b === void 0
      ? void 0
      : _b.removeChild(ele)
...
const getComputedToken = (originToken, overrideToken, theme, format) => {
  ...
  let mergedDerivativeToken = _extends(
    _extends({}, derivativeToken),
    overrideToken
  )
  if (format) {
...
function useCacheToken(theme, tokens) {
  let option =
    arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : ref({})
  const style = useStyleInject()
  const mergedToken = computed(() => _extends({}, ...tokens.value))
  const tokenStr = computed(() => flattenToken(mergedToken.value))
  const overrideTokenStr = computed(() =>
    flattenToken(option.value.override || EMPTY_OVERRIDE)
  )
  const cachedToken = useClientCache(
    'token',
    computed(() => [
      option.value.salt || '',
      theme.value.id,
...
function prepare() {
  ...
  if (!cachePathMap) {
    ...
    if (canUseDom$1()) {
      ...
      if (inlineMapStyle) {
        fromCSSFile = false
        ;(_a = inlineMapStyle.parentNode) === null || _a === void 0
          ? void 0
          : _a.removeChild(inlineMapStyle)
      }
...
function injectSelectorHash(key2, hashId, hashPriority) {
  ...
  const keys = key2.split(',').map(k2 => {
    ...
    const htmlElement =
      ((_a = firstPath.match(/^\w+/)) === null || _a === void 0
        ? void 0
        : _a[0]) || ''
...
const parseStyle = function (interpolation) {
  let config =
    arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}
  let { root, injectHash, parentSelectors } =
  ...
  function parseKeyframes(keyframes) {
    ...
    if (!effectStyle[animationName]) {
      ...
      effectStyle[animationName] =
        `@keyframes ${keyframes.getName(hashId)}${parsedStr}`
    }
  }
...
`
      ...
      const mergedStyle = transformers.reduce((prev2, trans) => {
        var _a
        return (
          ((_a = trans === null || trans === void 0 ? void 0 : trans.visit) ===
            null || _a === void 0
            ? void 0
            : _a.call(trans, prev2)) || prev2
        )
      }, style)
      Object.keys(mergedStyle).forEach(key2 => {
        ...
        if (
          typeof value === 'object' &&
          value &&
          (key2 !== 'animationName' || !value._keyframe) &&
          ...
          const actualValue =
            (_a = value === null || value === void 0 ? void 0 : value.value) !==
              null && _a !== void 0
              ? _a
              : value
          if (
...
function useStyleRegister(info, styleFn) {
  ...
  useClientCache(
    ...
    () => {
      ...
      const { hashPriority, container, transformers, linters, cache } =
        styleContext.value
  ...
  return _extends(
    _extends(
      _extends(
        _extends(
          _extends(
            _extends(_extends({}, token2), colorPalettes),
...
function formatToken(derivativeToken) {
  ...
  const aliasToken = _extends(
    _extends(_extends({}, mergedToken), {
...
function genComponentStyleHook(component, styleFn, getDefaultToken) {
  return _prefixCls => {
    const prefixCls = computed(() =>
      _prefixCls === null || _prefixCls === void 0 ? void 0 : _prefixCls.value
    )
    ...
    return [
      useStyleRegister(componentInfo, () => {
        const { token: proxyToken, flush } = statisticToken(token2.value)
        const defaultComponentToken =
          typeof getDefaultToken === 'function'
            ? getDefaultToken(proxyToken)
            : getDefaultToken
        const mergedComponentToken = _extends(
          _extends({}, defaultComponentToken),
          token2.value[component]
        )
        ...
        return [
          genCommonStyle(token2.value, prefixCls.value),
          styleInterpolation,
        ]
      }),
      hashId,
...
function useToken() {
  const designTokenContext = inject(
    DesignTokenContextKey,
    computed(() => globalDesignTokenApi.value || defaultConfig)
  )
  const salt = computed(
    () => `${version}-${designTokenContext.value.hashed || ''}`
  )
  const mergedTheme = computed(
    () => designTokenContext.value.theme || defaultTheme
  )
  const cacheToken = useCacheToken(
    mergedTheme,
    computed(() => [seedToken, designTokenContext.value.token]),
    computed(() => ({
      salt: salt.value,
      override: _extends(
        {
  ...
  return [
    ...
    computed(() =>
      designTokenContext.value.hashed ? cacheToken.value[1] : ''
    ),
  ]
}
...
const Empty = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    ...
    const { direction, prefixCls: prefixClsRef } = useConfigInject(
      'empty',
      props
    )
    ...
    return () => {
      ...
      const _c = _extends(_extends({}, props), attrs),
        {
          image: mergedImage = ((_a = slots.image) === null || _a === void 0
            ? void 0
            : _a.call(slots)) || h$1(Empty$2),
          description = ((_b = slots.description) === null || _b === void 0
            ? void 0
            : _b.call(slots)) || void 0,
          imageStyle,
          class: className = '',
        } = _c,
        restProps = __rest$5(_c, [
          'image',
          'description',
          'imageStyle',
          'class',
      ...
      const image =
        typeof mergedImage === 'function' ? mergedImage() : mergedImage
      const isNormal =
        typeof image === 'object' &&
        'type' in image &&
        image.type.PRESENTED_IMAGE_SIMPLE
      return wrapSSR(
        createVNode(
          LocaleReceiver,
          {
...
const useConfigInject = (name, props) => {
  ...
  const configProvider = inject(
    configProviderKey,
    _extends(_extends({}, defaultConfigProvider), {
      renderEmpty: name2 =>
        h$1(DefaultRenderEmpty, {
          componentName: name2,
        }),
    })
  )
  const prefixCls = computed(() =>
    configProvider.getPrefixCls(name, props.prefixCls)
  )
  const direction = computed(() => {
    var _a, _b
    return (_a = props.direction) !== null && _a !== void 0
      ? _a
      : (_b = configProvider.direction) === null || _b === void 0
        ? void 0
        : _b.value
  })
  const iconPrefixCls = computed(() => {
    var _a
    return (_a = props.iconPrefixCls) !== null && _a !== void 0
      ? _a
      : configProvider.iconPrefixCls.value
  })
  const rootPrefixCls = computed(() => configProvider.getPrefixCls())
  const autoInsertSpaceInButton = computed(() => {
    var _a
    return (_a = configProvider.autoInsertSpaceInButton) === null ||
      _a === void 0
      ? void 0
      : _a.value
  })
  ...
  const getTargetContainer = computed(() => {
    var _a, _b
    return (_a = props.getTargetContainer) !== null && _a !== void 0
      ? _a
      : (_b = configProvider.getTargetContainer) === null || _b === void 0
        ? void 0
        : _b.value
  })
  const getPopupContainer = computed(() => {
    var _a, _b, _c
    return (_b =
      (_a = props.getContainer) !== null && _a !== void 0
        ? _a
        : props.getPopupContainer) !== null && _b !== void 0
      ? _b
      : (_c = configProvider.getPopupContainer) === null || _c === void 0
        ? void 0
        : _c.value
  })
  const dropdownMatchSelectWidth = computed(() => {
    var _a, _b
    return (_a = props.dropdownMatchSelectWidth) !== null && _a !== void 0
      ? _a
      : (_b = configProvider.dropdownMatchSelectWidth) === null || _b === void 0
        ? void 0
        : _b.value
  })
  const virtual = computed(() => {
    var _a
    return (
      (props.virtual === void 0
        ? ((_a = configProvider.virtual) === null || _a === void 0
            ? void 0
            : _a.value) !== false
  ...
  const autocomplete = computed(() => {
    var _a, _b, _c
    return (_a = props.autocomplete) !== null && _a !== void 0
      ? _a
      : (_c =
            (_b = configProvider.input) === null || _b === void 0
              ? void 0
  ...
  const disabled = computed(() => {
    var _a
    return (_a = props.disabled) !== null && _a !== void 0
      ? _a
      : disabledContext.value
  })
  const csp = computed(() => {
    var _a
  ...
  const wave = computed(() => {
    var _a, _b
    return (_a = props.wave) !== null && _a !== void 0
      ? _a
      : (_b = configProvider.wave) === null || _b === void 0
        ? void 0
        : _b.value
  })
  return {
...
function o(e2) {
  return (
    1 ==
      (null != (t2 = e2) &&
        'object' == typeof t2 &&
        false === Array.isArray(t2)) &&
    '[object Object]' === Object.prototype.toString.call(e2)
  )
  var t2
}
...
function l(e2) {
  var t2,
    n2 =
      null !== (t2 = null == e2 ? void 0 : e2.type) && void 0 !== t2 ? t2 : e2
  if (n2) {
...
var s = function (e2) {
    ...
    return (
      false !== o(e2) &&
      'function' == typeof (t2 = e2.constructor) &&
      false !== o((n2 = t2.prototype)) &&
      false !== n2.hasOwnProperty('isPrototypeOf')
    )
  },
  v = function (e2) {
    return e2
...
function k(e2, t2, n2) {
  ...
  if (O(c2)) {
    var v2 = u2.validator
    ;(v2 &&
      (v2 = null !== (f2 = (a2 = v2).__original) && void 0 !== f2 ? f2 : a2),
      (u2.validator = j(
        v2
          ? function (e3) {
...
function L(e2, t2) {
  if (
    (void 0 === t2 && (t2 = 'custom validation failed'),
    'function' != typeof e2)
  )
    throw new TypeError(
...
function getMotion$1(_ref) {
  let { prefixCls, animation, transitionName: transitionName2 } = _ref
  if (animation) {
    return {
...
const getTransitionGroupProps = function (transitionName2) {
  ...
  const transitionProps = transitionName2
    ? _extends(
        {
          name: transitionName2,
          appear: true,
  ...
  setup(props, _ref) {
    ...
    onUpdated(() => {
      nextTick(() => {
        var _a
        if (shouldRender.value) {
          ;(_a = props.didUpdate) === null || _a === void 0
            ? void 0
            : _a.call(props, props)
        }
      })
...
function generate(node2, key2, rootProps) {
  if (!rootProps) {
    return h$1(
      ...
      (node2.children || []).map(function (child, index) {
        return generate(
          child,
          ''.concat(key2, '-').concat(node2.tag, '-').concat(index)
        )
      })
    )
  }
...
var Icon = function Icon2(props, context) {
  ...
  var classObj =
    ((_classObj = {}),
    _defineProperty$a(_classObj, rootClassName.value, !!rootClassName.value),
    _defineProperty$a(_classObj, prefixCls.value, true),
    _defineProperty$a(
      _classObj,
      ''.concat(prefixCls.value, '-').concat(icon.name),
      Boolean(icon.name)
    ),
    _defineProperty$a(
      _classObj,
      ''.concat(prefixCls.value, '-spin'),
  ...
  return createVNode(
    ...
    [
      ...
      createVNode(InsertStyles, null, null),
    ]
  )
}
Icon.props = {
...
const Notice = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    ...
    const duration = computed(() =>
      props.duration === void 0 ? 4.5 : props.duration
    )
    ...
    return () => {
      ...
      const {
        prefixCls,
        closable,
        closeIcon = (_a = slots.closeIcon) === null || _a === void 0
          ? void 0
          : _a.call(slots),
        onClick,
        holder,
      ...
      const dataOrAriaAttributeProps = Object.keys(attrs).reduce(
        (acc, key2) => {
          if (
            key2.startsWith('data-') ||
            key2.startsWith('aria-') ||
            key2 === 'role'
          ) {
            acc[key2] = attrs[key2]
          }
...
const Notification$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    ...
    const add = (originNotice, holderCallback) => {
      ...
      const { maxCount: maxCount2 } = props
    ...
    return () => {
      ...
      const noticeNodes = notices.value.map((_ref3, index) => {
        let { notice: notice2, holderCallback } = _ref3
        const updateMark =
          index === notices.value.length - 1 ? notice2.updateMark : void 0
        const { key: key2, userPassKey } = notice2
        const { content } = notice2
        const noticeProps = _extends(
          _extends(
            _extends(
              {
                prefixCls,
...
Notification$1.newInstance = function newNotificationInstance(
  properties,
  callback
) {
  const _a = properties || {},
  ...
  const Wrapper = /* @__PURE__ */ defineComponent({
    ...
    setup(_props, _ref4) {
      ...
      return () => {
        ...
        const transitionName2 = hasTransitionName2
          ? customTransitionName
          : `${prefixCls.value}-${customTransitionName}`
        return createVNode(
          ConfigProvider,
          _objectSpread2(
            _objectSpread2({}, global2),
            {},
            {
  ...
  setup(props, _ref) {
    ...
    return () => {
      ...
      const noticeNodes = placementList.value.map(placement => {
        ...
        const classes =
          (_a2 = props.getClassName) === null || _a2 === void 0
            ? void 0
            : _a2.call(props, placement)
        const styles =
          (_b = props.getStyles) === null || _b === void 0
            ? void 0
            : _b.call(props, placement)
        const noticeNodesForPlacement = noticesForPlacement.map(
          (_ref2, index) => {
            let { notice: notice2, holderCallback } = _ref2
            const updateMark =
              index === notices.value.length - 1 ? notice2.updateMark : void 0
            const { key: key2, userPassKey } = notice2
            const { content } = notice2
            const noticeProps = _extends(
              ...
              ),
              {
                ...
                onClose: noticeKey => {
                  var _a3
                  remove2(noticeKey)
                  ;(_a3 = notice2.onClose) === null || _a3 === void 0
                    ? void 0
                    : _a3.call(notice2)
                },
                onClick: notice2.onClick,
        ...
        function onAfterLeave() {
          ...
          ;(_a3 = props.onAllRemoved) === null || _a3 === void 0
            ? void 0
            : _a3.call(props)
        }
        return createVNode(
...
function useNotification$1() {
  ...
  const {
    ...
    shareConfig = __rest$3(rootConfig, [
      'getContainer',
      'motion',
      'prefixCls',
      'maxCount',
  ...
  setup(props, _ref) {
    ...
    return () => {
      ...
      return createVNode(
        ...
        {
          class: classNames(
            `${props.prefixCls}-custom-content`,
...
const Holder$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    let { expose } = _ref
    var _a, _b
    const { getPrefixCls, getPopupContainer } = useConfigInject(
      'message',
      props
    )
    ...
    const getStyles = () => {
      var _a2
      const top =
        (_a2 = props.top) !== null && _a2 !== void 0 ? _a2 : DEFAULT_OFFSET$1
      return {
        left: '50%',
        transform: 'translateX(-50%)',
    ...
    const getClassName = () =>
      classNames(hashId.value, props.rtl ? `${prefixCls.value}-rtl` : '')
    const getNotificationMotion = () => {
      var _a2
      return getMotion$1({
        prefixCls: prefixCls.value,
        animation:
          (_a2 = props.animation) !== null && _a2 !== void 0 ? _a2 : `move-up`,
        transitionName: props.transitionName,
      })
    }
    const mergedCloseIcon = createVNode(
      'span',
      {
    ...
    const [api2, holder] = useNotification$1({
      ...
      duration:
        (_a = props.duration) !== null && _a !== void 0
          ? _a
          : DEFAULT_DURATION$1,
      getContainer:
        (_b = props.staticGetContainer) !== null && _b !== void 0
          ? _b
...
const genNotificationPlacementStyle = token2 => {
  const { componentCls, width, notificationMarginEdge } = token2
...
const genNotificationStyle = token2 => {
  ...
  return [
    // ============================ Holder ============================
    {
      [componentCls]: _extends(
        _extends(
          _extends(_extends({}, resetComponent(token2)), {
            position: 'fixed',
      ...
      [noticeCls]: {
        ...
        [`${noticeCls}-close`]: {
          ...
          '&:hover': {
            ...
            backgroundColor: token2.wireframe
              ? 'transparent'
...
function PureContent(_ref) {
  let { prefixCls, icon, type, message, description, btn } = _ref
  let iconNode = null
  if (icon) {
    iconNode = createVNode(
      'span',
      {
...
const Holder = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    ...
    const getStyles = placement => {
      var _a, _b
      return getPlacementStyle(
        placement,
        (_a = props.top) !== null && _a !== void 0 ? _a : DEFAULT_OFFSET,
        (_b = props.bottom) !== null && _b !== void 0 ? _b : DEFAULT_OFFSET
      )
    }
    const [, hashId] = useStyle$1(prefixCls)
    const getClassName = () =>
      classNames(hashId.value, {
    ...
    const [api2, holder] = useNotification$1({
      ...
      getContainer: () => {
        var _a, _b
        return (
          ((_a = props.getPopupContainer) === null || _a === void 0
            ? void 0
            : _a.call(props)) ||
          ((_b = getPopupContainer.value) === null || _b === void 0
            ? void 0
            : _b.call(getPopupContainer)) ||
          document.body
        )
      },
      maxCount: props.maxCount,
...
function useInternalNotification(notificationConfig) {
  ...
  const open = config => {
    ...
    const { open: originOpen, prefixCls, hashId } = holderRef.value
    const noticePrefixCls = `${prefixCls}-notice`
    const { message, description, icon, type, btn, class: className } = config,
      restConfig = __rest$1(config, [
        'message',
        'description',
        'icon',
        'type',
  ...
  const destroy = key2 => {
    var _a, _b
    if (key2 !== void 0) {
      ;(_a = holderRef.value) === null || _a === void 0
        ? void 0
        : _a.close(key2)
    } else {
  ...
  Notification$1.newInstance(
    {
      ...
      style: getPlacementStyle(
        placement,
        top !== null && top !== void 0 ? top : defaultTop,
        bottom !== null && bottom !== void 0 ? bottom : defaultBottom
      ),
      appContext,
      getContainer: getContainer2,
      closeIcon: _ref2 => {
        let { prefixCls: prefixCls2 } = _ref2
        const closeIconToRender = createVNode(
          'span',
          {
...
function getStyle(globalPrefixCls, theme) {
  ...
  if (theme.primaryColor) {
    ...
    variables['primary-color-deprecated-l-35'] = formatColor(primaryColor, c2 =>
      c2.lighten(35)
    )
    variables['primary-color-deprecated-l-20'] = formatColor(primaryColor, c2 =>
      c2.lighten(20)
    )
    variables['primary-color-deprecated-t-20'] = formatColor(primaryColor, c2 =>
      c2.tint(20)
    )
    variables['primary-color-deprecated-t-50'] = formatColor(primaryColor, c2 =>
      c2.tint(50)
    )
    variables['primary-color-deprecated-f-12'] = formatColor(primaryColor, c2 =>
      c2.setAlpha(c2.getAlpha() * 0.12)
    )
    const primaryActiveColor = new TinyColor(primaryColors[0])
    variables['primary-color-active-deprecated-f-30'] = formatColor(
      primaryActiveColor,
      c2 => c2.setAlpha(c2.getAlpha() * 0.3)
    )
  ...
  const cssList = Object.keys(variables).map(
    key2 => `--${globalPrefixCls}-${key2}: ${variables[key2]};`
  )
  return `
  :root {
    ${cssList.join('\n')}
...
function useTheme(theme, parentTheme) {
  const themeConfig = computed(
    () => (theme === null || theme === void 0 ? void 0 : theme.value) || {}
  )
  const parentThemeConfig = computed(() =>
    themeConfig.value.inherit === false ||
  ...
  const mergedTheme = computed(() => {
    if (!(theme === null || theme === void 0 ? void 0 : theme.value)) {
      return parentTheme === null || parentTheme === void 0
        ? void 0
        : parentTheme.value
    }
    ...
    Object.keys(theme.value.components || {}).forEach(componentName => {
      mergedComponents[componentName] = _extends(
        _extends({}, mergedComponents[componentName]),
        theme.value.components[componentName]
      )
    })
    return _extends(
      _extends(_extends({}, parentThemeConfig.value), themeConfig.value),
      {
        token: _extends(
          _extends({}, parentThemeConfig.value.token),
...
const ConfigProvider = /* @__PURE__ */ defineComponent({
  ...
  setup(props, _ref) {
    ...
    const iconPrefixCls = computed(
      () =>
        props.iconPrefixCls ||
        parentContext.iconPrefixCls.value ||
        defaultIconPrefixCls
    )
    const shouldWrapSSR = computed(
      () => iconPrefixCls.value !== parentContext.iconPrefixCls.value
    )
    const csp = computed(() => {
      var _a
      return (
        props.csp ||
        ((_a = parentContext.csp) === null || _a === void 0 ? void 0 : _a.value)
      )
    })
    ...
    const mergedTheme = useTheme(
      ...
      computed(() => {
        var _a
        return (_a = parentContext.theme) === null || _a === void 0
          ? void 0
          : _a.value
      })
    )
    ...
    const autoInsertSpaceInButton = computed(() => {
      var _a, _b
      return (_a = props.autoInsertSpaceInButton) !== null && _a !== void 0
        ? _a
        : (_b = parentContext.autoInsertSpaceInButton) === null || _b === void 0
          ? void 0
          : _b.value
    })
    const locale$12 = computed(() => {
      ...
      return (
        props.locale ||
        ((_a = parentContext.locale) === null || _a === void 0
          ? void 0
          : _a.value)
      )
    })
    ...
    const direction = computed(() => {
      ...
      return (
        props.direction ||
        ((_a = parentContext.direction) === null || _a === void 0
          ? void 0
          : _a.value)
      )
    })
    const space = computed(() => {
      var _a, _b
      return (_a = props.space) !== null && _a !== void 0
        ? _a
        : (_b = parentContext.space) === null || _b === void 0
          ? void 0
          : _b.value
    })
    const virtual = computed(() => {
      var _a, _b
      return (_a = props.virtual) !== null && _a !== void 0
        ? _a
        : (_b = parentContext.virtual) === null || _b === void 0
          ? void 0
          : _b.value
    })
    const dropdownMatchSelectWidth = computed(() => {
      ...
      return (_a = props.dropdownMatchSelectWidth) !== null && _a !== void 0
        ? _a
        : (_b = parentContext.dropdownMatchSelectWidth) === null ||
            _b === void 0
          ? void 0
          : _b.value
    })
    const getTargetContainer = computed(() => {
      var _a
      return props.getTargetContainer !== void 0
        ? props.getTargetContainer
        : (_a = parentContext.getTargetContainer) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const getPopupContainer = computed(() => {
      var _a
      return props.getPopupContainer !== void 0
        ? props.getPopupContainer
        : (_a = parentContext.getPopupContainer) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const pageHeader = computed(() => {
      var _a
      return props.pageHeader !== void 0
        ? props.pageHeader
        : (_a = parentContext.pageHeader) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const input = computed(() => {
      var _a
      return props.input !== void 0
        ? props.input
        : (_a = parentContext.input) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const pagination = computed(() => {
      var _a
      return props.pagination !== void 0
        ? props.pagination
        : (_a = parentContext.pagination) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const form = computed(() => {
      var _a
      return props.form !== void 0
        ? props.form
        : (_a = parentContext.form) === null || _a === void 0
          ? void 0
          : _a.value
    })
    const select = computed(() => {
      var _a
      return props.select !== void 0
        ? props.select
        : (_a = parentContext.select) === null || _a === void 0
          ? void 0
          : _a.value
    })
    ...
    const wave = computed(() => {
      ...
      return (_a = props.wave) !== null && _a !== void 0
        ? _a
        : (_b = parentContext.wave) === null || _b === void 0
          ? void 0
          : _b.value
    })
    const configProvider = {
      ...
      theme: computed(() => {
        var _a, _b
        return (_a = mergedTheme.value) !== null && _a !== void 0
          ? _a
          : (_b = parentContext.theme) === null || _b === void 0
            ? void 0
            : _b.value
      }),
    ...
    const validateMessagesRef = computed(() => {
      ...
      if (locale$12.value) {
        validateMessages =
          ((_a = locale$12.value.Form) === null || _a === void 0
            ? void 0
            : _a.defaultValidateMessages) ||
          ((_b = localeValues.Form) === null || _b === void 0
            ? void 0
    ...
    const renderProvider = legacyLocale => {
      var _a, _b
      let childNode = shouldWrapSSR.value
        ? wrapSSR(
            (_a = slots.default) === null || _a === void 0
              ? void 0
              : _a.call(slots)
...
const _hoisted_1$1 = {
  class: 'folder-content flex h-screen bg-gray-900 text-white',
}
const _hoisted_2 = {
...
const _hoisted_16 = {
  class: 'text-xs font-medium text-white truncate flex items-center',
}
...
const _hoisted_24 = {
  class:
    'w-48 h-72 bg-gray-800 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden',
}
...
const _hoisted_29 = {
  class: 'px-2 pb-3 rounded text-xs text-gray-300 font-mono',
}
const _hoisted_30 = {
  key: 0,
  class: 'mb-4',
...
const _hoisted_53 = {
  class: 'border border-gray-500 rounded-lg overflow-hidden',
}
  ...
  setup(__props, { emit: __emit }) {
    ...
    const startResize = e2 => {
      ...
      const handleMouseMove = e22 => {
        ...
        if (
          newWidth >= minPanelWidth.value &&
          newWidth <= maxPanelWidth.value
        ) {
    ...
    const readDir = async () => {
      ...
      try {
        ...
        const loadingMessage = api$1.loading(
          '正在扫描目录及子目录，请稍候...',
          0
        )
        try {
          ...
          if (dirResult.success) {
            ...
            if (allItems.length > 0) {
              const existingPaths = new Set(
                fileData.value.map(item => item.path)
              )
    ...
    const isFanartFile = fileName => {
      ...
      return (
        hasImageExtension &&
        (name.includes('fanart') || name.includes('backdrop'))
      )
    }
    ...
    const hasPosterFile = item => {
      ...
      return item.files.some(
        file => !file.isDirectory && isPosterFile(file.name)
      )
    }
    ...
    const parseNfoContent = content => {
      ...
      try {
        ...
        if (originalTitleMatch)
          info.originaltitle = originalTitleMatch[1].trim()
    ...
    const processedItems = computed(() => {
      ...
      topLevelFolders.forEach(folder => {
        const folderFiles = visibleFiles.filter(
          f2 => f2.isFile && f2.path.startsWith(folder.path + '/')
        )
        ...
        createElementBlock('div', _hoisted_1$1, [
          createBaseVNode(
            'div',
            {
              ...
              class:
                'border-r border-gray-700 overflow-y-auto flex-shrink-0 relative rounded-md glass-panel',
            },
            [
                  ...
                  createBaseVNode('div', _hoisted_4, [
                    createBaseVNode(
                      'button',
                      {
                        ...
                        class:
                          'p-2 rounded-lg bg-blue-600 bg-opacity-70 hover:bg-opacity-90 disabled:bg-gray-600 disabled:bg-opacity-70 text-white transition-all duration-200 backdrop-blur-sm shadow-lg',
                        title: '刷新',
                      },
                      _cache[1] ||
                        (_cache[1] = [
                    ...
                    _cache[2] ||
                      (_cache[2] = createBaseVNode(
                        'span',
                        { class: 'text-xs text-white text-opacity-80 mt-1' },
                        '刷新',
                        -1
                      )),
                  ]),
                  createBaseVNode('div', _hoisted_6, [
                    createBaseVNode(
                      'button',
                      {
                        ...
                        disabled: dirLoading.value,
                        class:
                          'p-2 rounded-lg bg-green-600 bg-opacity-70 hover:bg-opacity-90 disabled:bg-gray-600 disabled:bg-opacity-70 text-white transition-all duration-200 backdrop-blur-sm shadow-lg',
                    ...
                    [
                      createBaseVNode(
                        'button',
                        {
                          ...
                          class:
                            'p-2 rounded-lg bg-red-600 bg-opacity-70 hover:bg-opacity-90 text-white transition-all duration-200 backdrop-blur-sm shadow-lg',
                          title: '清除缓存',
                        },
                        _cache[6] ||
                          (_cache[6] = [
                ...
                : (openBlock(),
                  createElementBlock('div', _hoisted_11, [
                    ...
                    createElementBlock(
                      Fragment,
                      null,
                      renderList(processedItems.value, (item, index) => {
                        return (
                          openBlock(),
                          createElementBlock(
                            'div',
          ...
          createBaseVNode(
            'div',
            {
              ...
              class:
                'w-0.5 bg-gray-600 hover:bg-blue-500 cursor-col-resize transition-colors flex-shrink-0 relative group',
            },
            _cache[13] ||
              (_cache[13] = [
                createBaseVNode(
                  ...
                  {
                    class:
                      'absolute inset-y-0 left-1/2 transform -translate-x-1/2 w-3 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity',
                  },
                  [
                    createBaseVNode('div', {
                      class: 'w-0.5 h-8 bg-white rounded-full',
                    }),
                  ],
                  -1
                ),
          ...
          createBaseVNode('div', _hoisted_20, [
            selectedItem.value && fanartImageDataUrl.value
              ? (openBlock(),
                createElementBlock(
                  'div',
                  {
            ...
            !selectedItem.value
              ...
              : (openBlock(),
                createElementBlock('div', _hoisted_22, [
                  createBaseVNode('div', _hoisted_23, [
                    ...
                    createBaseVNode('div', _hoisted_27, [
                      createBaseVNode(
                        'h1',
                        _hoisted_28,
                        toDisplayString(selectedItem.value.name),
                        1
                      ),
                      createBaseVNode(
                        'div',
                        _hoisted_29,
                        toDisplayString(selectedItem.value.path),
                        1
                      ),
                      movieInfo.value
                        ? (openBlock(),
                          createElementBlock('div', _hoisted_30, [
                            createBaseVNode('div', _hoisted_31, [
                              createBaseVNode('div', _hoisted_32, [
                                movieInfo.value.director
                                  ? (openBlock(),
                                    createElementBlock('div', _hoisted_33, [
                                      _cache[16] ||
                                        (_cache[16] = createBaseVNode(
                                          'span',
                                          {
                                            class:
                                              'text-gray-400 w-16 flex-shrink-0',
                                          },
                                          '导演：',
                                          -1
                                        )),
                                  ...
                                  : createCommentVNode('', true),
                                movieInfo.value.actor &&
                                movieInfo.value.actor.length > 0
                                  ? (openBlock(),
                                    createElementBlock('div', _hoisted_35, [
                                      _cache[17] ||
                                        (_cache[17] = createBaseVNode(
                                          'span',
                                          {
                                            class:
                                              'text-gray-400 w-16 flex-shrink-0',
                                      ...
                                      createBaseVNode('span', _hoisted_36, [
                                        ...
                                        movieInfo.value.actor.length > 3
                                          ? (openBlock(),
                                            createElementBlock(
                                              'span',
                                              _hoisted_37,
                                  ...
                                  : createCommentVNode('', true),
                                movieInfo.value.genre &&
                                movieInfo.value.genre.length > 0
                                  ? (openBlock(),
                                              ...
                                              return (
                                                ...
                                                createElementBlock(
                                                  'span',
                                                  {
                                                    ...
                                                    class:
                                                      'px-1 py-0.5 bg-blue-400 text-blue-100 rounded',
                                                  },
                                                  toDisplayString(g2),
                                                  1
                                                )
                                  ...
                                  ? (openBlock(),
                                    createElementBlock('div', _hoisted_42, [
                                      ...
                                      createBaseVNode(
                                        'span',
                                        _hoisted_43,
                                        toDisplayString(movieInfo.value.rating),
                                        1
                                      ),
                                    ]))
                                  : createCommentVNode('', true),
                                movieInfo.value.country
                                  ? (openBlock(),
                                    createElementBlock('div', _hoisted_44, [
                                      _cache[21] ||
                                  ...
                                  : createCommentVNode('', true),
                                movieInfo.value.studio
                                  ? (openBlock(),
                                    createElementBlock('div', _hoisted_46, [
                                      _cache[22] ||
                                        (_cache[22] = createBaseVNode(
                                          'span',
                                          {
                                            class:
                                              'text-gray-400 w-16 flex-shrink-0',
                                      ...
                                      createBaseVNode(
                                        'span',
                                        _hoisted_47,
                                        toDisplayString(movieInfo.value.studio),
                                        1
                                      ),
                                    ]))
                                  : createCommentVNode('', true),
                                movieInfo.value.premiered
                                  ? (openBlock(),
                              ...
                              movieInfo.value.plot
                                ? (openBlock(),
                                  createElementBlock('div', _hoisted_50, [
                                    _cache[24] ||
                                      (_cache[24] = createBaseVNode(
                                        'h4',
                                        {
                                          class:
                                    ...
                                    createBaseVNode(
                                      'p',
                                      _hoisted_51,
                                      toDisplayString(movieInfo.value.plot),
                                      1
                                    ),
                                  ]))
                                : createCommentVNode('', true),
                      ...
                      selectedItem.value.files
                        ? (openBlock(),
                          createElementBlock('div', _hoisted_52, [
                            _cache[30] ||
                              (_cache[30] = createBaseVNode(
                                'h3',
                                {
                                  class:
                                    'text-md font-semibold text-white mb-2',
                                },
                                '文件列表',
                                -1
                              )),
                            createBaseVNode('div', _hoisted_53, [
                              _cache[29] ||
                                (_cache[29] = createBaseVNode(
                                  'div',
                                  {
                                    class:
                                      'grid grid-cols-12 gap-4 p-2 text-sm font-medium text-gray-300',
                                  },
                                  [
                                    createBaseVNode(
                                      'div',
                              ...
                              createElementBlock(
                                Fragment,
                                null,
                                renderList(selectedItem.value.files, file => {
                                  return (
                                    openBlock(),
                                    createElementBlock(
                                      'div',
                                      {
                                        key: file.path,
                                        class:
                                          'grid grid-cols-12 gap-4 p-2 border-t border-gray-500 hover:bg-gray-750 transition-colors',
                                      },
                                      [
...
const FolderContent = /* @__PURE__ */ _export_sfc(_sfc_main$1, [
  ['__scopeId', 'data-v-5d3cde41'],
])
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return (
        openBlock(),
        createElementBlock('div', _hoisted_1, [createVNode(FolderContent)])
      )
    }
  },
})
const App = /* @__PURE__ */ _export_sfc(_sfc_main, [
  ['__scopeId', 'data-v-11328dbb'],
])
