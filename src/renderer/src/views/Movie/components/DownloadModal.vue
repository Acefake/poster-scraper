  ...
  <Transition name="modal-fade">
    ...
    >
      ...
      <div
        ...
        style="background: rgba(10,12,18,0.96); backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 24px 64px rgba(0,0,0,0.7);"
        @click.stop
      >
        <!-- 头部 -->
        <div class="flex items-center justify-between px-5 py-4 border-b border-white/8">
          <div>
            ...
            <div class="text-xs text-gray-500 mt-0.5 truncate max-w-[280px]">{{ avid || '请选择下载源' }}</div>
          </div>
          <button
            @click="$emit('cancel')"
        ...
        <div class="px-5 py-4 space-y-2 overflow-y-auto max-h-[360px]">
          <p class="text-[11px] text-gray-500 mb-3">选择下载源（按推荐权重排序）</p>
          <div
            ...
            :class="selected === site.downloaderName
          ...
          >
            ...
            <div
              ...
              :class="selected === site.downloaderName ? 'border-blue-400 bg-blue-400' : 'border-gray-600'"
            >
              <div v-if="selected === site.downloaderName" class="w-1.5 h-1.5 rounded-full bg-white" />
            </div>
            <div class="flex-1 min-w-0">
              <div class="text-xs font-medium text-white/90">{{ site.downloaderName }}</div>
              <div class="text-[10px] text-gray-500">{{ site.domain }}</div>
            </div>
            <!-- 权重标签 -->
            <span
              class="text-[9px] px-1.5 py-0.5 rounded font-mono"
              :class="site.weight > 0 ? 'bg-green-900/60 text-green-400' : 'bg-gray-800 text-gray-500'"
            >{{ site.weight > 0 ? '推荐' : '禁用' }}</span>
          </div>
        </div>

        <!-- 结果提示 -->
        <div v-if="resultMsg || queueStatus" class="mx-5 mb-3 px-3 py-2 rounded-lg bg-black/40 border border-white/8 text-[11px] text-gray-300 font-mono space-y-1">
        ...
        <div class="px-5 py-4 border-t border-white/8 flex items-center gap-3">
          <button
            ...
            :class="selected && !downloading
              ? 'bg-blue-600 hover:bg-blue-500 text-white'
              : 'bg-white/5 text-gray-500 cursor-not-allowed'"
          >{{ downloading ? '提交中...' : '开始下载' }}</button>
          <button
            @click="$emit('cancel')"
...
watch(() => props.visible, (v) => {
  if (v) {
    ...
    selected.value = first?.downloaderName ?? (sortedSites.value[0]?.downloaderName ?? '')
  } else {
    stopPolling()
  }
})
