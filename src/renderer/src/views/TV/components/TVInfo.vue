    ...
    <div v-else-if="tvInfo">
      ...
      <div class="flex flex-wrap gap-2 mb-3">
        ...
        <span v-if="tvInfo.rating" class="info-badge">
          <svg class="w-3 h-3 text-yellow-400 inline mr-0.5" fill="currentColor" viewBox="0 0 20 20">
        ...
        <span v-if="tvInfo.number_of_seasons" class="info-badge">{{ tvInfo.number_of_seasons }} 季</span>
        <span v-if="tvInfo.number_of_episodes" class="info-badge">{{ tvInfo.number_of_episodes }} 集</span>
      </div>

      <!-- 类型 -->
      <div v-if="tvInfo.genre && tvInfo.genre.length > 0" class="flex flex-wrap gap-1.5 mb-3">
        <span v-for="g in tvInfo.genre" :key="g" class="genre-tag">{{ g }}</span>
      </div>
      ...
      <div class="space-y-1.5 text-[13px]">
        ...
        <div v-if="tvInfo.actor && tvInfo.actor.length > 0" class="text-gray-300">
          <span class="text-gray-500 mr-1">演员</span> {{ tvInfo.actor.slice(0, 6).join(' / ') }}
        </div>
      ...
      <!-- 简介 -->
      <p v-if="tvInfo.plot" class="text-xs text-gray-300/80 leading-relaxed mt-3">{{ tvInfo.plot }}</p>
    </div>
