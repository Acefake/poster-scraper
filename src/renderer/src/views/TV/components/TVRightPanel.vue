<template>
  ...
  <template v-if="selectedItem.isSeasonFolder">
    ...
    <div class="glass-card p-5 mb-5">
      <div class="flex items-start gap-6">
        <div class="flex flex-col items-center">
          <div class="w-40 rounded-xl overflow-hidden flex-shrink-0 shadow-lg ring-1 ring-white/10">
            <img
            ...
            />
            <div v-else class="w-full aspect-[2/3] bg-white/5 flex items-center justify-center">
        ...
        <div class="flex-1 min-w-0">
          <div class="text-[11px] text-gray-400/80 uppercase tracking-widest mb-1 font-medium">
            {{ currentTVShow?.name ?? '' }}
          </div>
          <h1 class="text-2xl font-bold text-white mb-3 leading-tight drop-shadow">
            {{ selectedItem.name }}
          </h1>
          <div class="flex items-center gap-4 text-xs text-gray-400 mb-3">
            <span v-if="currentSeasonMeta?.air_date" class="bg-white/5 px-2 py-0.5 rounded">{{ currentSeasonMeta.air_date.substring(0, 4) }}</span>
            <span class="bg-white/5 px-2 py-0.5 rounded">{{ episodeList.length }} 集</span>
          </div>
          <p v-if="currentSeasonMeta?.overview" class="text-xs text-gray-300/90 leading-relaxed">
    ...
    <div class="glass-card p-5">
      <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">
        集列表 <span class="ml-1 text-gray-500">{{ episodeList.length }}</span>
      </h2>
      <div class="space-y-2">
        <div
        ...
        >
          <div class="w-28 h-16 rounded-lg overflow-hidden flex-shrink-0 ring-1 ring-white/10 bg-white/5">
            <img
              ...
              @error="handleImageError"
            />
            <div v-else class="w-full h-full flex items-center justify-center text-sm font-bold text-gray-500/60">
              {{ ep.episodeNumber != null ? 'E' + String(ep.episodeNumber).padStart(2, '0') : '??' }}
            </div>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-[13px] text-gray-200 truncate">{{ ep.name }}</div>
  ...
  <template v-else>
    ...
    <div class=" p-5 mb-5">
      <div class="flex items-start gap-6">
        <div class="flex flex-col items-center">
          <div class="w-44 rounded-xl overflow-hidden flex-shrink-0 shadow-lg ring-1 ring-white/10">
            <img
              v-if="posterUrl"
              :src="posterUrl"
              alt="海报"
        ...
        <div class="flex-1 min-w-0">
          <h1 class="text-3xl font-bold text-white mb-2 drop-shadow-lg leading-tight">
            {{ selectedItem.name }}
          </h1>
        ...
        >
          <div class="w-24 flex-shrink-0 self-stretch bg-white/[0.02]">
            <img
              ...
              alt=""
            />
            <div v-else class="w-full h-full min-h-[120px] flex items-center justify-center">
          ...
          <div class="flex-1 min-w-0 py-3 pr-4">
            <div class="flex items-center gap-2 mb-1">
              <span class="font-semibold text-white text-sm">{{ season.name }}</span>
              <span v-if="season.season_number === 0" class="text-[10px] bg-yellow-500/15 text-yellow-400 px-1.5 py-0.5 rounded-md font-bold">特辑</span>
            </div>
            <div class="flex items-center gap-3 text-[11px] text-gray-400 mb-2">
              <span v-if="season.air_date" class="bg-white/5 px-1.5 py-0.5 rounded">{{ season.air_date.substring(0, 4) }}</span>
              <span class="bg-white/5 px-1.5 py-0.5 rounded">{{ season.episode_count ?? getLocalEpisodeCount(season.season_number) }} 集</span>
            </div>
            <p v-if="season.overview" class="text-xs text-gray-300/80 leading-relaxed line-clamp-3">{{ season.overview }}</p>
...
const currentSeasonMeta = computed((): SeasonInfo | undefined => {
  ...
  return props.tvInfo?.seasons?.find(s => s.season_number === props.selectedItem.seasonNumber)
})

/** 当前季海报 data URL */
const currentSeasonPoster = computed((): string =>
  props.seasonPosters?.[props.selectedItem.path] ?? ''
)
...
const seasonList = computed((): SeasonInfo[] => {
  ...
  if (props.tvInfo?.seasons?.length) {
    return [...props.tvInfo.seasons].sort((a, b) => a.season_number - b.season_number)
  }
