...
class Sracper:
    ...
    def scrape(self, avid: str) -> Optional[AVMetadata]:
        ...
        if gid_uc_pairs:
            metadata.magnets = self._fetch_magnets(metadata.avid, gid_uc_pairs)
            logger.info(f"fetch magnets succ: {len(metadata.magnets)} found")
    ...
    def _extract(self, html: str) -> Optional[AVMetadata]:
        try:
            ...
            # 8. 提取 gid/uc JS 变量用于后续获取磁力链接
            gid_match = re.search(r'var\s+gid\s*=\s*(\d+)', html)
            uc_match = re.search(r'var\s+uc\s*=\s*(\d+)', html)
            if gid_match and uc_match:
                metadata._gid_uc_pairs = [(gid_match.group(1), uc_match.group(1))]
            else:
                metadata._gid_uc_pairs = []
            logger.debug(f"gid/uc pairs: {metadata._gid_uc_pairs}")
    ...
    def _fetch_magnets(self, avid: str, gid_uc_pairs: list) -> list:
        """通过 Ajax 接口获取磁力链接列表"""
        magnets = []
        for gid, uc in gid_uc_pairs[:5]:  # 最多取前5个种子
            try:
                ajax_url = f"https://{self.domain}/ajax/uncledatoolsbyajax.php"
                params = {
                    'lang': 'zh',
                    'img': '',
                    'gid': gid,
                ...
                resp = requests.get(
                    ...
                    headers={**headers, 'Referer': f'https://{self.domain}/{avid}', 'X-Requested-With': 'XMLHttpRequest'},
                ...
                html_frag = resp.text
                # 按 <tr> 分块，每块提取一条磁力
                tr_blocks = re.split(r'<tr\s', html_frag)
                for block in tr_blocks:
                    # 从 href 属性取完整磁力链接（含 dn=）
                    href_match = re.search(r'href="(magnet:[^"]+)"', block)
                    if not href_match:
                        continue
                    full_magnet = href_match.group(1)
                    # 只保留 btih hash（标准磁力格式）
                    btih_match = re.search(r'(magnet:\?xt=urn:btih:[0-9a-fA-F]+)', full_magnet)
                    magnet = btih_match.group(1) if btih_match else full_magnet
                    # 从 dn= 参数取文件名
                    dn_match = re.search(r'[?&]dn=([^"&]+)', full_magnet)
                    name = dn_match.group(1) if dn_match else ''
                    # 文件大小
                    size_match = re.search(r'([\d.]+\s*(?:GB|MB|KB))', block, re.IGNORECASE)
                    size = size_match.group(1).strip() if size_match else ''
                    # 日期
                    date_match = re.search(r'(\d{4}-\d{2}-\d{2})', block)
                    date = date_match.group(1) if date_match else ''
                    magnets.append({'magnet': magnet, 'name': name, 'size': size, 'date': date})
                    logger.debug(f"magnet: {name} {size} {date} {magnet[:60]}...")
                if magnets:
                    break  # 只需请求一次（一个 gid 返回所有磁力）
            except Exception as e:
                logger.error(f"获取磁力失败 gid={gid}: {e}")
        return magnets
